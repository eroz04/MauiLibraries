...
public class AppleSignInClient
{
    ...
    public async Task<AppleUser> ExchangeTokenAsync(string code)
    {
        ...
        var idJwtToken = JwtToken.Decode(respProperties["id_token"]);
        var accessToken = respProperties?["access_token"];
        var refreshToken = respProperties?["refresh_token"];

        string email = idJwtToken.Payload.ContainsKey("email") ? idJwtToken.Payload["email"]?.ToString() : null;
        string name = idJwtToken.Payload.ContainsKey("name") ? idJwtToken.Payload["name"]?.ToString() : null;

        // Validate id token
        if (!idJwtToken.Issuer.Equals(AppleJwtUrl))
            throw new ProtocolViolationException($"Invalid id_token issuer received: Expected `{AppleJwtUrl}` but observed `{idJwtToken.Issuer}`");

        if (!idJwtToken.AccessTokenHash.Equals(Util.Sha256AtHash(accessToken)))
            throw new ProtocolViolationException("Access Token Hash did not match actual access token's hash");

        if (!idJwtToken.Algorithm.Equals("RS256"))
            throw new ProtocolViolationException($"Invalid id_token algorithm returned: Expected `RS256` but observed `{idJwtToken.Algorithm}`");

        // Verify audience claim
        if (!idJwtToken.Audience.Equals(ServerId))
            throw new ProtocolViolationException($"The id_token 'aud' claim does not match the provided clientId value.")
;
        if (idJwtToken.Expiration < DateTime.UtcNow)
